...
function calwat(){
    ...
    for(var i = 1; i < table.rows.length; i++)
    {
        for(var j = 1; j < 5 ; j++)
        {
            if (j==1){ sum = sum + (4*parseFloat(table.rows[i].cells[j].innerHTML));}
            if (j==2){ sum = sum + (9*parseFloat(table.rows[i].cells[j].innerHTML));}
            if (j==3){ sum = sum + (4*parseFloat(table.rows[i].cells[j].innerHTML));}
            if (j==4){ waa = waa + (0.001*parseFloat(table.rows[i].cells[j].innerHTML));}
